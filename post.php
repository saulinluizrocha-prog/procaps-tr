...
if ($_POST) {
    ...
    $tracking['sub_1']         = isset($_GET['sub_1'])         ? (string) $_GET['sub_1']        : (isset($_POST['sub_1'])        ? (string) $_POST['sub_1']        : '');
    $tracking['sub_2']         = isset($_GET['sub_2'])         ? (string) $_GET['sub_2']        : (isset($_POST['sub_2'])        ? (string) $_POST['sub_2']        : '');
    $tracking['sub_3']         = isset($_GET['sub_3'])         ? (string) $_GET['sub_3']        : (isset($_POST['sub_3'])        ? (string) $_POST['sub_3']        : '');
    $tracking['sub_4']         = isset($_GET['sub_4'])         ? (string) $_GET['sub_4']        : (isset($_POST['sub_4'])        ? (string) $_POST['sub_4']        : '');
    $tracking['sub_5']         = isset($_GET['sub_5'])         ? (string) $_GET['sub_5']        : (isset($_POST['sub_5'])        ? (string) $_POST['sub_5']        : '');
    $tracking['utm_campaign']  = isset($_GET['utm_campaign'])  ? (string) $_GET['utm_campaign'] : (isset($_POST['utm_campaign']) ? (string) $_POST['utm_campaign'] : '');
    $tracking['utm_source']    = isset($_GET['utm_source'])    ? (string) $_GET['utm_source']   : (isset($_POST['utm_source'])   ? (string) $_POST['utm_source']   : '');
    $tracking['utm_medium']    = isset($_GET['utm_medium'])    ? (string) $_GET['utm_medium']   : (isset($_POST['utm_medium'])   ? (string) $_POST['utm_medium']   : '');
    $tracking['utm_term']      = isset($_GET['utm_term'])      ? (string) $_GET['utm_term']     : (isset($_POST['utm_term'])     ? (string) $_POST['utm_term']     : '');
    $tracking['utm_content']   = isset($_GET['utm_content'])   ? (string) $_GET['utm_content']  : (isset($_POST['utm_content'])  ? (string) $_POST['utm_content']  : '');
    $tracking['gclid']         = isset($_GET['gclid'])         ? (string) $_GET['gclid']        : (isset($_POST['gclid'])        ? (string) $_POST['gclid']        : '');
    ...
    try {
        ...
        // Normaliza telefone para formato turco (905XXXXXXXXX)
        $phoneClean = preg_replace('/\D/', '', $phone);
        if (preg_match('/^0090/', $phoneClean))      { $phoneClean = substr($phoneClean, 4); }
        elseif (preg_match('/^90/', $phoneClean))    { $phoneClean = substr($phoneClean, 2); }
        elseif (preg_match('/^0/', $phoneClean))     { $phoneClean = substr($phoneClean, 1); }
        $phoneNormalized = '90' . $phoneClean;

        // Mandatory params validation — fall back to raw if needed, never lose lead
        if (!$name) { $name = 'Cliente'; }
        if (!$phoneNormalized) { $phoneNormalized = $phoneClean ?: $phone; }
        ...
        $postFields = [
            ...
            'offer_id'      => $offerId,
            'country_code'  => 'TR',
            'base_url'      => isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : 'https://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'],
            'referrer'      => isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : 'https://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'],
        ...
        // Evita deprecated warning no PHP 8+
        if (PHP_MAJOR_VERSION < 8) { curl_close($curl); }

        // Decode json to array
        $responseArray = json_decode($response, true);

        // Monta query string de tracking para a success page
        $trackingQuery = '';
        foreach ($tracking as $k => $v) {
            if ($v !== '') { $trackingQuery .= '&' . urlencode($k) . '=' . urlencode($v); }
        }

        // Sempre redireciona para a success page — nunca perde o lead
        $successUrl = '/tr-success.html?name=' . urlencode($name) . '&phone=' . urlencode($phoneNormalized) . $trackingQuery;
        header('Location: ' . $successUrl);
        die('');

    } catch (Exception $e) {
        // Mesmo em erro, vai para success (nunca perde o lead)
        $successUrl = '/tr-success.html?name=' . urlencode($name ?? '') . '&phone=' . urlencode($phone ?? '');
        header('Location: ' . $successUrl);
        die('');
    }
}
